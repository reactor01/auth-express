...
exports.signup = catchAsync(async (req, res, next) => {
	...
	if (req.body.email._value || req.body.password._value) {
		username = req.body.username._value
		email = req.body.email._value
		password = req.body.password._value
		passwordConfirm = req.body.passwordConfirm._value
	} else {
...
exports.login = catchAsync(async (req, res, next) => {
	...
	// Check logs array lenght
	const logLenght = await user.checkLenght()
	...
	if (logLenght < 10) {
		await User.findOneAndUpdate(
			...
			{
				$push: {
					lastLoginDateArr: log,
				},
			},
		)
	} else {
		await User.findOneAndUpdate({ email }, { $pop: { lastLoginDateArr: -1 } })
		await User.findOneAndUpdate(
			{ email },
			{
				$push: {
...
// exports.auto = catchAsync(async (req, res, next) => {
...
exports.isLoggedIn = async (req, res, next) => {
	...
	// let token = ''
	// if(req.body.token._value) {
	//     token = req.body.token._value
	// } else {
...
exports.check = catchAsync(async (req, res, next) => {
	try {
		...
		return next(new AppError('Something get wrong!!!', 401))
	}
})
